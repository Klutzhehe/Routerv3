...
def generate_curriculum_dataset(base_dir: str, num_boards_per_stage: int = 20):
    ...
    print(f"Generating Stage 1 (Basics: 2-4 nets) in {stage1_dir}...")
    ...
    print(f"Generating Stage 2 (Corridors: 5-8 nets) in {stage2_dir}...")
    for i in range(num_boards_per_stage):
        num_nets = 5 + (i % 4)
    ...
    print(f"Generating Stage 3 (Full Production: 6 nets + 2 diff pairs + 2 length groups) in {stage3_dir}...")
    for i in range(num_boards_per_stage):
