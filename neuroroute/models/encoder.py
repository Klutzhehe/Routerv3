...
native-resolution crop around each head (`HeadCropEncoder`). That split is the
...
class HeadCropEncoder(nn.Module):
    ...
    def forward(self, field: torch.Tensor, head_pos: torch.Tensor, head_mask: torch.Tensor) -> torch.Tensor:
        """field (B, C, L, H, W); head_pos (B, K, 3) as (layer, y, x).
        ...
        ys = head_pos[..., 1].clamp(0, H - 1).unsqueeze(-1) + off.view(1, 1, -1)   # (B, K, crop)
        xs = head_pos[..., 2].clamp(0, W - 1).unsqueeze(-1) + off.view(1, 1, -1)
        ...
        crop = crop * head_mask.reshape(-1, 1, 1, 1, 1).float()
